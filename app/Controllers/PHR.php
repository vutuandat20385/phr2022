...
class PHR extends BaseController {
	...
	public function d4uGroup(){
		...
		$data['form_importModal'] = $this->form->form_importGroupModal($formData);
	...
	public function d4uCovid(){
		$data['panelTitle'] = 'Danh sách Kết quả Test COVID';
        $data['user'] 	= session()->get('user');
		...
		$formData = array(
			'title' 			=> 'THÊM BỆNH KẾT QUẢ TEST COVID',
			'default_send_SMS'	=> $this->setting->getSettingValue(['settingType'=> 'sms', 'settingName' => DEFAULT_SMS_TEST_COVID]),
			'action'			=> 'trang-quan-tri/benh-an/d4u-test-covid/import'
		);
		$data['form_importModal'] = $this->form->form_importTestCovidModal($formData);
        ...
        $allPHR = $this->phr->getAllD4UCovidPHR($dataAllPHR);
        ...
        if(!empty($allPHR)){
            ...
            $data['listCovidTest'] = $this->phr->getAllD4UCovidPHR($dataPHR);

            foreach($data['listCovidTest'] as $k => $value){
                $data['listCovidTest'][$k]['index'] = $totalPHR - $start - $k;
            }
            ...
            $data['listCovidTest'] = false;
        }
        

        $data = $this->getAfterLoginLayout($data, 'Trang quản trị | Danh sách Bệnh án Khách đoàn | Doctor4U', 'AfterLogin/pages/phr/d4uCovid');
		return view('AfterLogin/main', $data);
	}
				...
				// Điện di Huyết Sắc Tố
					...
					if(isset($examInfo['HST'])){
						...
						foreach($HST_key as  $k => $value){
							...
							if($indexValue != '' && $indexValue != null){
								$data['huyet_sac_to'][$value] = $this->phr->showIndex($indexValue, $shortName, $gender, $k+1);
							}
						}
					}else{
						$data['huyet_sac_to'] = false;
	...
	public function detailGroup($visitId){
		$data['panelTitle'] = 'Chi tiết bệnh án';
        $data['user'] 	= session()->get('user');
			...
			if($data['phrInfo']['Occupation'] != 'auto'){
				...
				// Hóa sinh miễn dịch
					...
					if(isset($examInfo['hoaSinhMienDich'])){
						...
						foreach($hoaSinhMienDich_key as  $h => $value){
							$gender = $data['phrInfo']['gender'];
							$shortName = $value;
							$indexValue = $hoaSinhMienDich[$value];
							if($indexValue != '' && $indexValue != null){
								$data['hoa_sinh_mien_dich'][$value] = $this->phr->showIndex($indexValue, $shortName, $gender, $h+1);
							}
						}
					}else{
						$data['hoa_sinh_mien_dich'] = false;
	...
	public function editTestCovidStatus(){
		$data = array(
			'id' => $this->request->getVar('id')
		);

		$this->phr->updateTestCovidRecord($data, 'status');
		return true;
	}

	public function editTestCovidRecord(){
		
		$data = array(
			'id' 		=> $this->request->getVar('edit_id'),
			'date' 		=> $this->request->getVar('edit_testDate'),
			'result' 	=> $this->request->getVar('edit_testResult'),
			'type' 		=> $this->request->getVar('edit_testType'),
		);

		log_message('error',$data['id']);
		log_message('error',$data['date']);
		log_message('error',$data['result']);
		log_message('error',$data['type']);

		$this->phr->updateTestCovidRecord($data,'record');
		return redirect()->to('/trang-quan-tri/benh-an/d4u-test-covid');
	}

	public function uploadTestCovidResult(){
		...
		$page=(int)(($this->request->getVar('page')!==null)?$this->request->getVar('page'):1);
			...
			$target 		= './public/TestCovidResult';
			$timeLabel 		= time().'_'.date('Ymd');
			$newName 		= str_replace('.'.$ext,'',strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '_', $file->getName())))).'_'.$timeLabel.'.'.$ext;
			
			$file->move( $target , $newName);
    
            $data = array(
				'id' => $id,
				'file_result' => json_encode([
					'fileResult' => $newName,
					'ext'  => $ext
					])
				);
    
            $updateResult = $this->phr->updateResult($data);  

			// get DateResult
			$recordResult = $this->phr->getTestCovidResultRecord($id);
			$date = date('d-m-Y',strtotime($recordResult['date']));
			// log_message('error', $date);
			 
			if($updateResult){
				$checkSMS			= $this->request->getVar('checkSMS');
				$testInfo			= $this->phr->getTestInfo($id);

				$phone				= $testInfo['phone_number'];
				...
				$userCheck = $this->checkUserApi($dataCheckUser);
				$createDate = $userCheck['date_created'];
				if(strtotime($createDate) >= strtotime(date('Y-m-d'))){
					$settingName = CONTENT_SMS_UPLOADFILE_COVID_NEW;
				}else
					$settingName = CONTENT_SMS_UPLOADFILE_COVID_OLD;
				if($checkSMS == 'sms'){
					$ketqua = $testInfo['result'];
					// Send SMS đến SĐT bệnh nhân
					$msg = $this->phr->getSMSContent($settingName, 'sms', ['hospital' => $hospital, 'date' => $date, 'ketqua' => $ketqua]);
				...
				$this->sendNotification($phone,$content);
			}  
			return redirect()->to('/trang-quan-tri/benh-an/d4u-test-covid?page='.$page);
	}
}
